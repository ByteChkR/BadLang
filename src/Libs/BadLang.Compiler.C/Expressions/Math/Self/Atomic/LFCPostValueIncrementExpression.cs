...
public class LFCPostValueIncrementExpression : LFCExpression
{
    private readonly LFCExpression Left;

    public LFCPostValueIncrementExpression(LFSourcePosition position, LFCExpression left) : base(position)
    {
        Left = left;
    }
    ...
    {
        return new LFCPostValueIncrementExpression(Position, rebuilder(Left));
    }
}