...
public static class CodeSectionWriterExtensions
{
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.AddF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.AddF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.DivF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.DivF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.ModF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.ModF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.MulF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.MulF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    public static void Push(this CodeSectionWriter writer, int size)
    {
        while (size > 0)
            ...
            {
                writer.Emit(OpCode.PushI32, BitConverter.GetBytes(0));
                size -= 4;
            }
            else if (size >= 2)
            {
    ...
    public static void Push(this CodeSectionWriter writer, BadCType type, decimal v)
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.PushI32, BitConverter.GetBytes((float) v));
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.PushI64, BitConverter.GetBytes((double) v));
        else writer.Push(type, (long) v);
    }

    private static void Push(this CodeSectionWriter writer, BadCType type, long v)
    {
        switch (type.Size)
        {
            case 1:
            ...
            case 8:
                writer.Emit(OpCode.PushI64, BitConverter.GetBytes(v));
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.SubF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.SubF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.BranchGreaterF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.BranchGreaterF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.BranchGreaterOrEqualF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.BranchGreaterOrEqualF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.BranchLessF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.BranchLessF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
    ...
    {
        if (type.PrimitiveType == BadCPrimitiveTypes.F32)
            writer.Emit(OpCode.BranchLessOrEqualF32, Array.Empty<byte>());
        else if (type.PrimitiveType == BadCPrimitiveTypes.F64)
            writer.Emit(OpCode.BranchLessOrEqualF64, Array.Empty<byte>());
        else
            switch (type.Size)
            {
                case 1:
